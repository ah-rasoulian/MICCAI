...
class CrossEntropy():
    ...
    def __call__(self, probs: Tensor, target: Tensor) -> Tensor:
        ...
        loss = - einsum("bkxyz,bkxyz->", mask, log_p)
        loss /= mask.sum() + 1e-10
...
class GeneralizedDice():
    ...
    def __call__(self, probs: Tensor, target: Tensor) -> Tensor:
        ...
        w: Tensor = 1 / ((einsum("bkxyz->bk", tc).type(torch.float32) + 1e-10) ** 2)
        intersection: Tensor = w * einsum("bkxyz,bkxyz->bk", pc, tc)
        union: Tensor = w * (einsum("bkxyz->bk", pc) + einsum("bkxyz->bk", tc))
...
class DiceLoss():
    ...
    def __call__(self, probs: Tensor, target: Tensor) -> Tensor:
        ...
        intersection: Tensor = einsum("bcxyz,bcxyz->bc", pc, tc)
        union: Tensor = (einsum("bkxyz->bk", pc) + einsum("bkxyz->bk", tc))
...
class HausdorffLoss():
    ...
    def __call__(self, probs: Tensor, target: Tensor) -> Tensor:
        ...
        multipled = einsum("bkxyz,bkxyz->bkxyz", delta, dtm)
...
class FocalLoss():
    ...
    def __call__(self, probs: Tensor, target: Tensor) -> Tensor:
        ...
        loss = - einsum("bkxyz,bkxyz,bkxyz->", w, mask, log_p)
        loss /= mask.sum() + 1e-10

...
def main():
    ...
    train_loader = DataLoader(train_ds, batch_size=16, collate_fn=image_label_collate)
    ...
    loss_fn = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([6], device='cuda'))
    opt = Adam(model.parameters())
    epoch = 1
    while not early_stopping.early_stop:
        m = train_one_epoch(model, opt, loss_fn, train_loader, valid_loader)
        print(f"epoch {epoch}: train-loss:{m['train_cfm'].get_mean_loss()}, valid_loss:{m['valid_cfm'].get_mean_loss()}, "
